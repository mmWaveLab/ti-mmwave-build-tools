...
awk -F '\t' '
  ...
  END { exit ok ? 0 : 1 }
' "$repo_dir/config/starter-demo-profiles.tsv"
...
awk -F '\t' '
  ...
  END { exit ok ? 0 : 1 }
' "$repo_dir/docs/catalog/toolbox-oob-demo-profiles.tsv"
...
awk -F '\t' '
  ...
  END { exit ok ? 0 : 1 }
' "$repo_dir/docs/catalog/toolbox-application-demo-profiles.tsv"
