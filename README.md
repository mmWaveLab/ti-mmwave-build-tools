...
SDK `mss-only` rows build the R4F artifact such as `*_mss.xer4f`. SDK
`mss-dss` rows build the flashable metaimage `.bin`. `xwr6843aop-mss-dss` is
tracked as a Toolbox projectspec profile because TI's SDK OOB AOP entry is MSS
only; it needs the Toolbox importer before it can be generated by
`create-mmwave-app`.
